...
import genreList from '../Home/genre.js'
import "./Genrepage.css"

function GenrePage({onSelectSong}) {
    ...
    return(
        <>
            {album && (
                        <div className = "genre-page" style={{background: `linear-gradient(to bottom, ${album.color} 40%,transparent, transparent )`}}>
                        <div className="genre-page-nav">
                                <img src={album.image} className="genre-pfp"/>
                                <div className="right-top-detail">
                                    <h1 className="text-8xl font-extrabold text-white">{album.name}</h1>
                                    <ul className="artist-featuring">
                                        {album.songs.map((albums, index) => {
                                            return(
                                            <li className="artist-featuring-list" key={index}>{albums.artistname}</li>
                                            )
                                        })}
                                    </ul>
